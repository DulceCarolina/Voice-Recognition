...
tc3=xcorr(t,audio3);
plot(t,audio3)
title('Señal de audio 3')
xlabel('Tiempo (t)')
ylabel('Amplitud')
subplot(5,2,9)
plot(t,audio4)
title('Señal de audio 4')
xlabel('Tiempo (t)')
ylabel('Amplitud')
...
%% SEGUNDA CORRELACIÓN
SCT1=xcorr(Tx,T1); %%SEGUNDA CORRELACIÓN
SCT2=xcorr(Tx,T2);
SCT3=xcorr(Tx,T3);
SCT4=xcorr(Tx,T4);
SCTx=xcorr(Tx,Tx);
%%GRÁFICAS 1%%
figure('Name','SEGUNDA CORRELACIÓN');
subplot(5,2,2)
plot(t2,SCTx) 
title('TX')
xlabel('Tiempo')
...
%% alinear señales
%-----------------------
[Xa,Ya,D]  = alignsignals(audiox,audiox);
[Xb,Yb,D1] = alignsignals(audiox,audio1);
[Xc,Yc,D2] = alignsignals(audiox,audio2);
[Xd,Yd,D3] = alignsignals(audiox,audio3);
[Xe,Ye,D4] = alignsignals(audiox,audio4);
figure('Name','SEÑALES ALINEADAS');
subplot(5,1,1)
plot (Xa)
hold
plot (Ya)
title('Señal x - señal x')
subplot(5,1,2)
plot (Xb)
hold
plot (Yc)
title('Señal x - señal 1')
subplot(5,1,3)
plot (Xc)
hold
plot (Yc)
title('Señal x - señal 2')
subplot(5,1,4)
plot (Xd)
hold
plot (Yd)
title('Señal x - señal 3')
subplot(5,1,5)
plot (Xe)
hold
plot (Ye)
title('Señal x - señal 4')
%-------------------
%% CORRELACION SEÑALES ALINEADAS
CSA1=xcorr(Xb,Yb); %%SEGUNDA CORRELACIÓN
CSA2=xcorr(Xc,Yc);
CSA3=xcorr(Xd,Yd);
CSA4=xcorr(Xe,Ye);
CSAx=xcorr(Xa,Ya);

CSAMAX = [max(CSA1),max(CSA2),max(CSA3),max(CSA4)]/max(CSAx);

%% %% GRÁFICAS DE BARRAS
figure('Name','Correlcion señales alineadas');
%figure('Name','Correlcion entre audio X y los comandos','NumberTitle','off');
subplot(1,3,1)
title('Frecuencia')
bar(CSAMAX)
